...
import { PkContainerSubItemReadinessEnum, PkDSetSubItemContainerMappingRequest, GlobalResponseObject } from "@xpparel/shared-models";
...
@Injectable()
export class PkDispatchReadyService {
  constructor(
  ...
  async putDSetSubItemInTheContainer(req: PkDSetSubItemContainerMappingRequest): Promise<GlobalResponseObject> {
    ...
    try {
      if (!req?.subItemBarcode?.length) {
        throw new ErrorResponse(41010, "Barcode's not provided");
      }
      if (!req.containerId) {
        throw new ErrorResponse(41011, "Container barcode not provided");
      }
      ...
      if (dSetSubItemRecs.length != barcodeArr.length) {
        throw new ErrorResponse(41012, `Few Sub items not found. Provided ${barcodeArr.length}. Existing ${dSetSubItemRecs.length}`);
      }
      // validation - 2
      ...
      if (!dSetContainerRec) {
        throw new ErrorResponse(41013, "No containers found");
      }
      ...
      for (const subItemRec of dSetSubItemRecs) {
        ...
        if (!containerIds.includes(Number(req.containerId))) {
          ...
          throw new ErrorResponse(41014, 'The container is not mapped with the current CUT');
        }
        ...
        if (subItemContMapRec?.dSetContainerId == req.containerId) {
          ...
          throw new ErrorResponse(41015, 'Already Sub Items mapped to the same container');
        }
        if (!subItemContMapRec || req.override) {
          ...
          newMapping.currentStage = PkContainerSubItemReadinessEnum.PUT;
          ...
          if (subItemContMapRec) {
            await this.dSetSubItemContainerMapRepository.update({ companyCode: companyCode, unitCode: unitCode, id: subItemContMapRec.id }, { isActive: false, updatedUser: username, currentStage: PkContainerSubItemReadinessEnum.REMOVED });
          }
        }
      }
      await transManager.completeTransaction();
      return new GlobalResponseObject(true, 41016, 'Sub Item Container Map successfully updated');
    } catch (error) {
  ...
  async removeDSetSubItemInTheContainer(req: PkDSetSubItemContainerMappingRequest): Promise<GlobalResponseObject> {
    const transManager = new GenericTransactionManager(this.dataSource);
    try {
      await transManager.startTransaction();
      if (!req?.subItemBarcode?.length) {
        throw new ErrorResponse(41017, "Barcode's not provided");
      }
      const { companyCode, unitCode, username } = req;
      ...
      if (subItemContMapRecs.length == 0) {
        throw new ErrorResponse(41018, "None of the barcodes are mapped to any container");
      }
      ...
      await transManager.completeTransaction();
      return new GlobalResponseObject(true, 41019, "Sub Item Container Map successfully unmapped");
    } catch (error) {
      await transManager.releaseTransaction();
      throw error;
    }
