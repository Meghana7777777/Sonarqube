...
import { ContainerWiseSubItemBarcodeModel, DSetBarcodeModel, DSetGetBarcodesRequest, DSetItemIdsRequest, DSetItemsBarcodesModel, DSetBarcodesResponse, DSetSubItemsBarcodesModel, DSetItemBarcodesResponse, DSetItemAttrEnum, DSetSubItemBarcodeDetailedModel } from "@xpparel/shared-models";
...
@Injectable()
export class DispatchReadyInfoService {
  constructor(
  ...
  // ENDPOINT
  async getDsetBarcodeInfo(req: DSetGetBarcodesRequest) : Promise<DSetBarcodesResponse>{
    const transManager = new GenericTransactionManager(this.dataSource);
       //Check DsetIds received from the request
    if(!req?.dSetIds?.length){
      throw new ErrorResponse(0,'No DSetIds Provided');
    }
    ...
    const dSetBarcodeModels:DSetBarcodeModel[]=[];
    if(dSetRecs.length != req.dSetIds.length){
      throw new ErrorResponse(0,'Some Disptach set ids does not exist');
    }
    const dSetItemsBarcodeModels:DSetItemsBarcodesModel[]=[];
    for(const records of dSetRecs){
      ...
      for(const dSetItem of dSetItems) {
        ...
        // call the mapped 
        let plannedBrcdInfo: { planBarcodes: string[]; planBarcodesInfo: DSetSubItemsBarcodesModel[] };
        let putInBarcodeList: { putInBagBarcodes: string[]; putInBagBarcodesInfo: DSetSubItemsBarcodesModel[]; }
        let containerWiseBarcodes :  { containerWiseBarcodesInfo: ContainerWiseSubItemBarcodeModel[]; }
        ...
        const dSetBarcodesModel = new DSetItemsBarcodesModel(dSetItem.id, dSetItem.printStatus, attrsModel, plannedBrcdInfo?.planBarcodes, plannedBrcdInfo?.planBarcodesInfo,putInBarcodeList?.putInBagBarcodes, putInBarcodeList?.putInBagBarcodesInfo, containerWiseBarcodes?.containerWiseBarcodesInfo);
        dSetItemsBarcodeModels.push(dSetBarcodesModel);
      }
      const dSetBarcodes = new DSetBarcodeModel(records.setNo, dSetItemsBarcodeModels);
      dSetBarcodeModels.push(dSetBarcodes);
    }
    return new DSetBarcodesResponse(true,11,'',dSetBarcodeModels);
  }


  // HELPER
  async getPlannedBarcodesListModels(dsetItemId: number, companyCode: string, unitCode: string, iNeedAllBarcodesDetailedList: boolean, throwErrorIfNoRec: boolean): Promise<{planBarcodes: string[], planBarcodesInfo: DSetSubItemsBarcodesModel[]}> {
    ...
    if(dSetItemBarcodesRecs.length == 0 && throwErrorIfNoRec){
      throw new ErrorResponse(0,'No DsetIds found');
    }
    ...
    if(subItemRecords.length == 0 && throwErrorIfNoRec){
      throw new ErrorResponse(0,'No DSet SubItems Ids found');
    }

    let subItemAttrMap: Map<number, DSetSubItemBarcodeDetailedModel>;
    ...
    const dSetSubItemsBarcodes : DSetSubItemsBarcodesModel[] = [];
    ...
    for(const subItemRec of subItemRecords){
      ...
      const dSetSubItemsBarcodesModel = new DSetSubItemsBarcodesModel(subItemRec.barcode,subItemRec.itemQuantity,subItemRec.id, barcodeDetailInfo);
  ...
  // ENDPOINT
  async getDsetItemBarcodeInfo(req: DSetGetBarcodesRequest): Promise<DSetItemBarcodesResponse> {
    const transManager = new GenericTransactionManager(this.dataSource);
       //Check DsetIds received from the request
    if(!req?.dSetItemIds?.length){
      throw new ErrorResponse(0,'No DSetItemIds Provided');
    }
     const { companyCode, unitCode, username } = req;
      const dSetItemsBarcodeModels:DSetItemsBarcodesModel[]=[];
      ...
      if(dSetItemsRecs.length != req.dSetIds.length){
        throw new ErrorResponse(0,'Some set Item Ids does not exists');
      }
    ...
    return new DSetItemBarcodesResponse(true,11,'',dSetItemsBarcodeModels);
  }

  getDsetItemAttrEnumModel(dSetItemAttrRec: DSetItemAttrEntity): {[k in DSetItemAttrEnum]?: string} {
    return {
      [DSetItemAttrEnum.CO]:  dSetItemAttrRec.l3,
      [DSetItemAttrEnum.MO]:  dSetItemAttrRec.l1,
      [DSetItemAttrEnum.MOL]:  dSetItemAttrRec.lt2,
      [DSetItemAttrEnum.PSTREF]:  dSetItemAttrRec.lm1,
      [DSetItemAttrEnum.VPO]:  dSetItemAttrRec.l4,
      [DSetItemAttrEnum.PNM]:  dSetItemAttrRec.lm2,
      [DSetItemAttrEnum.CNO]:  dSetItemAttrRec.l6,
      [DSetItemAttrEnum.CSNO]:  dSetItemAttrRec.l7,
      [DSetItemAttrEnum.DDT]:  dSetItemAttrRec.l8,
      [DSetItemAttrEnum.COL]:  dSetItemAttrRec.l9,
      [DSetItemAttrEnum.DEST]:  dSetItemAttrRec.l5,
      [DSetItemAttrEnum.STY]:  dSetItemAttrRec.l2,
      [DSetItemAttrEnum.COMPS]:  dSetItemAttrRec.lt1
    }
    // Keep this ref code here
    // const setItemAttrEnt = new DSetItemAttrEntity();
    // setItemAttrEnt.l1 = so;
  ...
  // HELPER
  async getPutInBagBarcodesListModels(dsetItemId: number, companyCode: string, unitCode: string, iNeedPutInBagBarcodeDetailedList:boolean,throwErrorIfNoRec: boolean): Promise<{putInBagBarcodes: string[], putInBagBarcodesInfo: DSetSubItemsBarcodesModel[]}>{
    ...
    if(containerMapRecords.length == 0 && throwErrorIfNoRec){
      throw new ErrorResponse(0, 'No Records Found in Container map');
    }
    ...
    for(const rec of containerMapRecords){
      ...
      const dSetSubItemsBarcodesModel = new DSetSubItemsBarcodesModel(rec.barcode, subItemInfo.itemQuantity, rec.id, barcodeDetailInfo);
      dSetSubItemsBarcodes.push(dSetSubItemsBarcodesModel);
    }
    return {
  ...
  async getBarcodeDetailedModel(dSetItemId: number, companyCode: string, unitCode: string): Promise<Map<number, DSetSubItemBarcodeDetailedModel>> {
    const subItemArrRecs = await this.dSetSubItemAttrRepo.find({ select: ['dSetSubItemRefId', 'l1', 'l2'], where: { dSetItemId: dSetItemId } });
    const subItemArrMap = new Map<number, DSetSubItemBarcodeDetailedModel>();
    // NOTE: this map is not for dSetSubItem id. Since we dont maintain the dSetSubItem id in this DSetSubItemAttrEntity
    subItemArrRecs.forEach(r => subItemArrMap.set(Number(r.dSetSubItemRefId), new DSetSubItemBarcodeDetailedModel(false, r.l1, r.l2)));
    return subItemArrMap;
  }

  // HELPER 
  async getBagWiseAbractModels(dsetItemId: number, companyCode: string, unitCode: string, iNeedBagWiseAbstractWithDetailedBarcodes:boolean, throwErrorIfNoRec: boolean ): Promise<{containerWiseBarcodesInfo: ContainerWiseSubItemBarcodeModel[]}>{
    console.log('helo');
    ...
    if(containerRecords.length == 0 && throwErrorIfNoRec ){
      throw new ErrorResponse(0,'Container Ids not found in Container table');
    }
    ...
    const containerWiseSubItemBarcodeModel : ContainerWiseSubItemBarcodeModel[] = [];
    for(const con of containerRecords){
      ...
      const containerWiseBarcodes = new ContainerWiseSubItemBarcodeModel(con.id, con.containerNumber, con.containerType, con.barcode, barcodesInCon, dSetSubItemsBarcodes);
