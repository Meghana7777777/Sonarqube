import { GlobalResponseObject, KMS_C_KnitOrderBundlingConfirmationIdRequest, PoKnitBundlingMoveToInvStatusEnum, ProcessTypeEnum } from "@xpparel/shared-models";
...
@Injectable()
export class KnitInvCreationService {
    ...
    async createKnitInvInRequestByConfirmationId(req: KMS_C_KnitOrderBundlingConfirmationIdRequest): Promise<GlobalResponseObject> {
        ...
        try {
            ...
            const confirmedBundles = await this.inHelperService.getKnitConfirmedBundlesForConfirmationId(confirmationId, processType, companyCode, unitCode);
            ...
            await this.inHelperService.sendAckToKnitSystemForInvReceived(companyCode, unitCode, username, confirmationId, PoKnitBundlingMoveToInvStatusEnum.MOVED_TO_INV);
    ...
    async deleteKnitInvInRequestByConfirmationId(req: KMS_C_KnitOrderBundlingConfirmationIdRequest): Promise<GlobalResponseObject> {
        const transManager = new GenericTransactionManager(this.dataSource);
        try {
            const {companyCode, unitCode, username, confirmationId, processType } = req;
            // first check if the inventory request created for the confirmation id
            ...
            await transManager.completeTransaction();
            await this.inHelperService.sendAckToKnitSystemForInvReceived(companyCode, unitCode, username, confirmationId, PoKnitBundlingMoveToInvStatusEnum.OPEN);
