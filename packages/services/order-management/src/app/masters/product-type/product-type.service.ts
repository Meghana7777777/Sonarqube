...
export class ProductTypeService {
  ...
  async mapCompsToProductType(reqModel: ProductTypeRequest): Promise<ProductTypeResponse> {
    ...
    try {
      ...
      await transManager.completeTransaction();
      return new ProductTypeResponse(true, 1012, `${reqModel.productTypes.some(p => p.id) ? "Product Type Updated" : "Product Type Created"} Successfully`, []);
    } catch (error) {
      await transManager.releaseTransaction();
      throw error;
    }
  ...
  async getProductDetailsByProductType(req: ProductTypeIdRequest): Promise<ProductTypeResponse> {

    const record = await this.productTypeRepo.findOne({ where: { productType: req.prodTypeCode } });
    const resultData: ProductTypeModel[] = [];

    const prodComps = await this.productTypeCompRepo.find({ where: { productTypeId: record.id } });
    const componentmodel: ProductTypeCompModel[] = [];
    prodComps.forEach(p => {
      componentmodel.push(new ProductTypeCompModel(p.componetName));
    })
    const eachRow = new ProductTypeModel(req.username, req.unitCode, req.companyCode, req.userId, record.id, record.productType, record.productType, 'refno', record.productDesc, componentmodel);
    resultData.push(eachRow);
