...
import { CommonRequestAttrs, KnitHeaderInfoResoponse, MC_MoNumberRequest, MoProductSubLineIdsRequest, MoPslIdsRequest, MoPslQtyInfoModel, MoPslQtyInfoResponse, OrderFeatures, OrderTypeEnum, PO_StyleInfoResponse, PoColorDetailsModel, PoDataSummaryResponse, PoProductCodeDetailsModel, PoSerialRequest, PoSizeDetailsModel, PoSummaryDataModel, PoSummaryResponse, ProcessingOrderCreationInfoModel, ProcessingOrderCreationInfoResponse, ProcessingOrderInfoModel, ProcessingOrderInfoRequest, ProcessingOrderInfoResponse, ProcessingOrderLineInfo, ProcessingOrderMoInfoModel, ProcessingOrderProductInfo, ProcessingOrderProductSubLineInfo, ProcessingOrderSerialRequest, ProcessTypeEnum, ProcessTypeWiseSizeResponse, ProductInfoResponse, SI_MoProductSubLineIdsRequest, StyleCodeRequest, StyleMoRequest, StyleProductCodeRequest, StyleProductFgColorResp } from "@xpparel/shared-models";
...
export class ProcessingOrderInfoService {
    ...
    */
    ...
    async getMOInfoForPrcOrdCreation(styleMoRequest: StyleMoRequest): Promise<ProcessingOrderCreationInfoResponse> {
        ...
        for (const moNumberOne of moNumber) {
            ...
            const eligiblePSLIds = await this.poslBundleService.getEligibleProductSubLinesToCreatePo(moNumberRequest, [ProcessTypeEnum.KNIT])
     ...
     * @returns 
    */
    async getKnitOrderCreatedStyles(reqObj: CommonRequestAttrs): Promise<PO_StyleInfoResponse> {
    ...
    */
    ...
    /**
     * Service to get Knit Order Info for Style and product from ui
     * Usually calls from UI to Display processing orders for given style and product code
     * @param reqObj 
     * @returns 
    */
    async getKnitOrderInfoByStyeAndProduct(reqObj: StyleProductCodeRequest): Promise<ProcessingOrderInfoResponse> {
        const { unitCode, companyCode } = reqObj;
    ...
    */
    ...
    async getKnitHeaderInfoData(processingSerial: number, companyCode: string, unitCode: string): Promise<KnitHeaderInfoResoponse> {
        const moHeaderInfo = await this.productSubLineFeaturesRepository.getKnitHeaderInfoData(processingSerial, companyCode, unitCode);
        const procDetails = await this.poRepo.findOne({where:{companyCode,unitCode,processingSerial:processingSerial}})
        moHeaderInfo.processingOrderDesc = procDetails.prcOrdDescription
    ...
    async getPoQtysInfoForMoPSLIds(reqObj: MoPslIdsRequest): Promise<MoPslQtyInfoResponse> {
        ...
        if(!moPslIds || moPslIds.length === 0 ){
            throw new ErrorResponse(0, "No mo product sub line ids found")
       
        }
        const moWiseSubLineIds = await this.poSubLineRepo.find({ where: { moProductSubLineId: In(moPslIds), unitCode, companyCode }, select: ['processType', 'processingSerial', 'quantity','moProductSubLineId'] })
