...
import { GlobalResponseObject, MC_BundleCountModel, MC_MoProcessTypeModel, MC_ProductSubLineBundleCountDetail, MOC_BundleDetail, MC_MoNumberRequest, MoProductSubLineIdAndQtyRequest, MoProductSubLineIdsRequest, PO_PoSerialRequest, ProcessingOrderProductSubLineInfo, ProcessingOrderSerialRequest, ProcessTypeEnum, ProductSubLineAndBundleDetailModel, ProductSubLineAndBundleDetailResponse, BundleGenStatusEnum, MC_PoPslbBundleDetailModel } from "@xpparel/shared-models";
...
export class PoSubLineBundleService {
    ...
    */
     ...
     * @returns 
    */
    async createBundlesForPo(reqObj: PO_PoSerialRequest): Promise<GlobalResponseObject> {
        ...
        try {
            ...
            if (poInfo.bundleGenStatus != BundleGenStatusEnum.IN_PROGRESS) {
                throw new ErrorResponse(0, 'Bundle already generated. Please check and try again')
            }
            ...
            for (const eachProcessType of routingGroupDetails) {
                for (const eachMoSubLine of moSubLineInfoWithMoNumber) {
                    ...
                    for (const eachBundle of moSubLineBundles) {
                        ...
                        poBundlesObj.bundleState = KnitOrderProductBundleStateEnum.OPEN;
                        ...
                        moSubLineObjAct.bundleDetails.push(new MOC_BundleDetail(eachBundle.bundleNumber, eachBundle.quantity, eachBundle.itemSku, eachMoSubLine.mo_product_sub_line_id));
                    }
                }
            }
            await manager.completeTransaction();
            await this.poRepo.update({processingSerial, unitCode, companyCode},{bundleGenStatus: BundleGenStatusEnum.COMPLETED});
