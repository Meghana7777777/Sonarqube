...
import { FabIssuingEntityEnum, GlobalResponseObject, KMS_C_JobMainMaterialReqIdRequest, KMS_R_KnitJobRequestedItemsResponse, KMS_R_KnitJobRequestedMaterialModel, MaterialDestinationTypeEnum, MaterialReqStatusEnum, PhItemCategoryEnum, RequestTypeEnum, WhFabReqStatusRequest, WhMatReqLineItemStatusEnum, WhMatReqLineStatusDisplayValue, WhMatReqLineStatusEnum, WhReqByObjectEnum, WhReqByObjectISSEnum, WMS_C_IssuanceIdRequest, WMS_C_KnitMaterialIssuanceRequest, WMS_C_WhKnitMaterialRequestIdRequest, WMS_MAINMAT_R_JobRequestedMainItemsModel, WMS_MAINMAT_R_JobRequestedMainMatModel, WMS_MAINMAT_R_JobRequestedMainMatResponse, WMS_R_IssuanceIdItemsModel, WMS_R_IssuanceIdItemsResponse } from "@xpparel/shared-models";
...
@Injectable()
export class WmsKnitItemRequestService {
    constructor(
        private dataSource: DataSource,
        private kmsService: KnittingJobMaterialAllocationService,
    ...
    async allocateKnitMaterialByRequestId(req: KMS_C_JobMainMaterialReqIdRequest): Promise<GlobalResponseObject> {
        ...
        try {
            ...
            const materialRes: KMS_R_KnitJobRequestedItemsResponse = await this.kmsService.getRequestedKnitMaterialForReqId(req);
            if (!materialRes.status)
                throw new ErrorResponse(materialRes.errorCode, materialRes.internalMessage);
            await transManager.startTransaction();

            const requestData: KMS_R_KnitJobRequestedMaterialModel = materialRes.data[0]
            ...
            whHeaderEnt.extRefEntityType = WhReqByObjectEnum.KNITTING;
            whHeaderEnt.reqMaterialType = PhItemCategoryEnum.YARN;
            ...
            for (const line of materialRes.data) {
                ...
                whLineEnt.matDestinationType = MaterialDestinationTypeEnum.KNIT_TABLE;
    ...
    // IN WMS
    async issueKnitMaterialForWhReqId(req: WMS_C_KnitMaterialIssuanceRequest): Promise<GlobalResponseObject> {
        ...
        try {
            ...
            const prefix = `${WhReqByObjectISSEnum.KNITTING}-${datePart}-`;
            ...
            await this.kmsService.updateIssuedKnitMaterialFromWms(new WMS_C_IssuanceIdRequest(req.username, req.unitCode, req.companyCode, req.userId, savedIssuanceEntity.id));
    ...
    // IN WMS
    // common for all knit, sps, pack trims
    async updateIssuedItemsHandlingStatusForIssuanceId(req: WMS_C_IssuanceIdRequest): Promise<GlobalResponseObject> {
        const issuanceLog = await this.whMatIssHeadRepo.count({ where: { id: req.issuanceId } });
        if (!issuanceLog) {
            throw new ErrorResponse(212, `Request doesn't exist with given details`)
        }
        await this.whMatIssHeadRepo.update({ id: req.issuanceId }, { reportedByExtSys: true })
        return new GlobalResponseObject(true, 13213, 'Request updated successfully');
    }

    // IN wms
    async getAllocatedKnitMaterialForWhRequestId(req: WMS_C_WhKnitMaterialRequestIdRequest): Promise<WMS_MAINMAT_R_JobRequestedMainMatResponse> {
        const itemDetails: WMS_MAINMAT_R_JobRequestedMainItemsModel[] = [];
        ...
        for (const line of whLines) {
            ...
            for (const item of items) {
                itemDetails.push(new WMS_MAINMAT_R_JobRequestedMainItemsModel(item.itemBarcode, itemDetailsMap.get(item.itemBarcode).relaxWidth, itemDetailsMap.get(item.itemBarcode).itemCode, itemDetailsMap.get(item.itemBarcode).itemDesc, item.reqQuantity, line.id, item.issuedQuantity));
            }
        }
        return new WMS_MAINMAT_R_JobRequestedMainMatResponse(true, 2131, 'Data retrieved successfully', [new WMS_MAINMAT_R_JobRequestedMainMatModel(req.whReqId.toString(), itemDetails)])
    }

    // IN wms
    async getAllocatedKnitMaterialForExtRefId(req: KMS_C_JobMainMaterialReqIdRequest): Promise<WMS_MAINMAT_R_JobRequestedMainMatResponse> {
        const { companyCode, unitCode } = req;
        const whHeaderData = await this.whHeaderRepo.findOne({ where: { extRefId: req.reqId, companyCode, unitCode } });
        if (!whHeaderData)
            throw new ErrorResponse(1213, 'Details not found with given criteria');
        return this.getAllocatedKnitMaterialForWhRequestId(new WMS_C_WhKnitMaterialRequestIdRequest(req.username, unitCode, companyCode, req.userId, whHeaderData.id));
    }

    async changeWhYarnReqStatus(req: WhFabReqStatusRequest): Promise<GlobalResponseObject> {
        let manager = new GenericTransactionManager(this.dataSource);
        try {
            // TODO: do all the pre validations
            const whFabReq = await this.whHeaderRepo.findOne({ where: { companyCode: req.companyCode, unitCode: req.unitCode, isActive: true, extReqNo: req.materialRequestNo, reqMaterialType: PhItemCategoryEnum.YARN } });
            if (!whFabReq) {
