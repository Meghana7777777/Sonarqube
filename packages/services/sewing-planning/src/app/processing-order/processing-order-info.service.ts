...
import { CommonRequestAttrs, MC_MoNumberRequest, MoPslIdsRequest, MoPslQtyInfoModel, MoPslQtyInfoResponse, OrderFeatures, OrderTypeEnum, PO_StyleInfoResponse, PoColorDetailsModel, PoDataSummaryResponse, PoProductCodeDetailsModel, PoSizeDetailsModel, PoSummaryDataModel, ProcessingOrderCreationInfoModel, ProcessingOrderCreationInfoResponse, ProcessingOrderInfoModel, ProcessingOrderInfoRequest, ProcessingOrderInfoResponse, ProcessingOrderLineInfo, ProcessingOrderMoInfoModel, ProcessingOrderProductInfo, ProcessingOrderProductSubLineInfo, ProcessingOrderSerialRequest, ProcessTypeEnum, ProductInfoResponse, SewingIJobNoRequest, SewitemCodeWiseConsumptionResponse, SewJobBundleSheetModel, SewJobBundleSheetResponse, SI_MoProductSubLineIdsRequest, SpsBundleSheetRequest, StyleCodeRequest, StyleMoRequest, StyleProductCodeRequest, StyleProductFgColorResp } from "@xpparel/shared-models";
...
import { ProcessTypeQueryResp } from "../entities/repository/query-response";
import { ProcessingSerialRes } from "../entities/repository/query-response/processing-serial.qry-res";
...
export class ProcessingOrderInfoService {
    constructor(
        ...
        private readonly poRoutingGroupRepo: PoRoutingGroupRepository,
        private readonly poWhJobMaterialRepository: PoWhJobMaterialRepository,
        private readonly poWhRequestLineRepository: PoWhRequestLineRepository,
        private readonly poWhRequestMaterialItemRepository: PoWhRequestMaterialItemRepository,
    ) {
    ...
    */
    ...
    private async getProcessingOrderProductInfo(line: PoLineEntity, req: ProcessingOrderInfoRequest): Promise<ProcessingOrderProductInfo[]> {
        ...
        console.log(poSublineRes, 'po sub line res')
        const productRefs = poSublineRes.map((v) => v.productRef)
        const products = await this.poProductRepo.find({ where: { productRef: In(productRefs), processingSerial: req.processingSerial } });
    ...
    private async getProcessingOrderProductSubLineInfo(
        ...
        const subLines = await this.poSubLineRepo.find({
            where: { processingSerial: poLineEntity.processingSerial, poLineId: poLineEntity.id, productRef: productRef }
        });
     ...
     * @returns 
    */
    async getSewOrderCreatedStyles(reqObj: CommonRequestAttrs): Promise<PO_StyleInfoResponse> {
    ...
    */
     ...
     * @returns 
    */
    async getSPSOrderInfoByStyeAndProduct(reqObj: StyleProductCodeRequest): Promise<ProcessingOrderInfoResponse> {
        const { unitCode, companyCode } = reqObj;
        ...
        for (const eachPo of poInfo) {
            const processingOrderInfo = await this.poRepo.findOne({ where: { processingSerial: eachPo, unitCode, companyCode, isActive: true }, select: ['processingSerial', 'prcOrdDescription'] });
            const poInfoObj = new ProcessingOrderInfoModel(processingOrderInfo.processingSerial, processingOrderInfo.prcOrdDescription, [], null, null, []);
            poInfoArray.push(poInfoObj);
        }
        return new ProcessingOrderInfoResponse(true, 0, 'Knit Order Info Retrieved Successfully', poInfoArray);
    }
    ...
    */
    ...
    async getPoQtysInfoForMoPSLIds(reqObj: MoPslIdsRequest): Promise<MoPslQtyInfoResponse> {
        ...
        const moWiseSubLineIds = await this.poSubLineRepo.find({ where: { moProductSubLineId: In(moPslIds), unitCode, companyCode, }, select: ['processingSerial', 'quantity'] })
        if (moWiseSubLineIds.length === 0) {
            throw new ErrorResponse(0, "No data found for given mo product sub line ids")
        }
        const distinctProcessingSerials: number[] = [...new Set(moWiseSubLineIds.map((v) => v.processingSerial))];
        const distinctProcessTypes: ProcessTypeQueryResp[] = await this.poRoutingGroupRepo.getDistinctProcessTypesForPoSerial(distinctProcessingSerials, unitCode, companyCode)

        if (distinctProcessTypes.length === 0) {
            throw new ErrorResponse(0, "No data found for given mo product sub line ids")
        }
        const moPslQtyInfoResponseArr: MoPslQtyInfoModel[] = []
        for (const eachProcessType of distinctProcessTypes) {
            for (const eachMoWiseSubLineId of moWiseSubLineIds) {
                const moPslQtyInfo = new MoPslQtyInfoModel(eachMoWiseSubLineId.moProductSubLineId, eachMoWiseSubLineId.quantity, eachProcessType.processType, eachMoWiseSubLineId.processingSerial)
    ...
    async getObjectWiseDataByJobNumber(req: SpsBundleSheetRequest): Promise<SewJobBundleSheetResponse> {
        try {
            const poWhJobMaterialData = await this.poWhJobMaterialRepository.getPoWhMaterialDataByJobNumber(req.jobNo, req.iNeedInventory, req.companyCode, req.unitCode);

            if (!poWhJobMaterialData.length) {
                throw new ErrorResponse(0, "No data found for the given job number.");
            }

            const poWhReqLineIds: number[] = [];
            await Promise.all(poWhJobMaterialData.map(async (each) => {
                const poWhReqLines = await this.poWhRequestLineRepository.find({ select: ['id'], where: { jobNumber: each.jobNumber, companyCode: req.companyCode, unitCode: req.unitCode, size: each.size, itemCode: each.itemCode, productRef: each.productRef, fgColor: each.fgColor } });
                poWhReqLineIds.push(...poWhReqLines.map(line => line.id));
            }));

            if (!poWhReqLineIds.length) {
                throw new ErrorResponse(0, "No request lines found for the given job number.");
            }

            const jobBundleDataResults = await Promise.all(
                poWhReqLineIds.map(id => this.poWhRequestMaterialItemRepository.getBundleBarcodeDetailsByPoWhRequestLineId(id, req.iNeedInventory, req.companyCode, req.unitCode))
            );
            const jobBundledata = jobBundleDataResults.flat();
            const objectCodeMap = new Map<string, SewJobBundleSheetModel>();
            jobBundledata.forEach((entry) => {
                if (!entry.objectCode) return;
                if (objectCodeMap.has(entry.objectCode)) {
                    const existing = objectCodeMap.get(entry.objectCode)!;
                    existing.issuedQty += entry.issuedQty || 0;
                    existing.requiredQty += entry.requiredQty || 0;
                    existing.allocatedQty += entry.allocatedQty || 0;
                } else {
                    objectCodeMap.set(entry.objectCode, { ...entry });
                }
            });

            const uniqueByObjectCode = Array.from(objectCodeMap.values());
            return new SewJobBundleSheetResponse(true, 1, "Data retrieved successfully", uniqueByObjectCode);
        } catch (error: any) {
            return new SewJobBundleSheetResponse(false, 0, "Error occurred", error.message || "Unexpected error");
        }
    }

    async getItemCodeWiseConsumptionByJobNumber(req: SpsBundleSheetRequest): Promise<SewitemCodeWiseConsumptionResponse> {
        try {
            const consumptionData = await this.poWhJobMaterialRepository.getPoWhMaterialDataByJobNumber(req.jobNo, req.iNeedInventory, req.companyCode, req.unitCode);
            if (!consumptionData.length) {
                throw new ErrorResponse(0, "No data found for the given job number.");
            }
            const uniqueEntriesMap = new Map<string, any>();
            for (const entry of consumptionData) {
                const key = `${entry.itemCode}-${entry.size}`;
            ...
            return new SewitemCodeWiseConsumptionResponse(true, 1, "Data retrieved successfully", uniqueData);
        } catch (error) {
            return new SewitemCodeWiseConsumptionResponse(false, 0, "Error occurred", error.message || "Unexpected error");
        }
    }
