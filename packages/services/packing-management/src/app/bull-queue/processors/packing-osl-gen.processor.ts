...
import { PackingBullJobNames, PackSerialRequest, SI_MoNumberRequest } from "@xpparel/shared-models";
import { Job } from "bull";
import { PKMSFgCreationService } from "../../pre-integrations/fg-creation.service";

@Processor(PackingBullJobNames.PACK_OSL_INFO_GEN)
@Injectable()
export class PKMS_OSL_GEN_Processor {
    constructor(
    ...
    @Process({ name: PackingBullJobNames.PACK_OSL_INFO_GEN })
    async processPopulateSewFgs(job: Job) {
        try {
            const req: SI_MoNumberRequest = job.data;
            return await this.service.createOslRefIdsForMo(req)
        } catch (error) {
            console.log(error);
