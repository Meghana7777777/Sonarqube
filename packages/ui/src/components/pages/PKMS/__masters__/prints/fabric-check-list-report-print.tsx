import { FgWhReqSubItemModel, FgWhSrIdPlIdsRequest, InsPhIdRequest, PackListCartoonIDs, RollsCheckListResponse } from "@xpparel/shared-models";
import { FabricInspectionInfoService, PKMSFgWarehouseService } from "@xpparel/shared-services";
import { Button } from "antd";
import { RollReqHeaderModel, RollsReqItemsModel } from "packages/libs/shared-models/src/ins/inspection-info/ins-checklist-model";
...
export const FabricCheckListReport = (props: CheckListIProps) => {
    const { plIds, userFromModal, title, packListCartoonIds,insId } = props;
    ...
    const [data, setData] = useState<RollReqHeaderModel[] | null>(null);
    ...
    const renderPrintDetails = async (plIds: number[], packListCartoonIds: PackListCartoonIDs[]) => {
        ...
        const reqObj = new FgWhSrIdPlIdsRequest(user?.userName, user?.orgData?.unitCode, user?.orgData?.companyCode, user?.userId, plIds, ' go here', true, true,
            true, true, true, [], cartoonId);

        try {
            const req = new InsPhIdRequest(user?.userName, user?.orgData?.unitCode, user?.orgData?.companyCode, user?.userId, plIds.map(String),[],insId);
            const whInfoForGivenPackList = await fabricInspectionInfoService.getCheckListPrintData(req);
    ...
    const getPoLocationSet = (req: RollsReqItemsModel[]) => {
        const poLocationMap = new Map<string, FgWhReqSubItemModel[]>();
        for (const eachSet of req) {
            for (const eachItem of eachSet.rollsInfo) {
                const location = eachItem.location ? eachItem.location : "On Floor";
                const key = `${eachSet.whReqItemAttrs.poNo}-${location}`
                if (!poLocationMap.has(key)) {
                    poLocationMap.set(key, [])
                }
                poLocationMap.get(key).push({
                    ...eachItem,
                    ctnQty: eachItem.rollQty || 0,
                });
            }
        }
        console.log(poLocationMap);
        const allRows = [];
        for (const [key, itemInfo] of poLocationMap) {

            const poNumber = key.split('-')[0];
            const location = key.split('-')[1];
            const ctnQty = itemInfo.reduce((pre, curr) => {
                                        ...
                                        <td style={{ border: "1px solid #000", padding: "10px" }}>
                                            <>
                                                <pre style={{ margin: 0 }}>
                                                    ...
                                                    {data.flatMap(item =>
                                                        item.whReqItems.flatMap(reqItem =>
                                                            reqItem.rollsInfo.map(item => item.rollQty || 0)
                                                        )
                                                ...
                                                <pre style={{ margin: 0 }}>
                                                    ...
                                                    {data.flatMap(item =>
                                                        item.whReqItems.flatMap(reqItem =>
                                                            reqItem.rollsInfo.map(item => (item.rollQty ? 1 : 0))
                                                        )
                                                    ).reduce((sum, ctn) => sum + ctn, 0)}
                                                </pre>
                                            </>
                                ...
                                <thead>
                                    <tr>
                                        <th colSpan={4} style={{ border: "1px solid #000", padding: "10px", textAlign: "center" }}>
                                            Items: {data.flatMap(item => item.whReqItems.flatMap(reqItem => reqItem.whReqItemAttrs.prodNames)).join(', ')}
                                        </th>
                                        <th colSpan={4} style={{ border: "1px solid #000", padding: "10px", textAlign: "center" }}>
                                    ...
                                    <tr>
                                        <th colSpan={4} style={{ border: "1px solid #000", padding: "10px", textAlign: "center" }}>
                                            Buyer Names: {data.flatMap(item => item.whReqItems.flatMap(reqItem => reqItem.whReqItemAttrs.buyers)).join(', ')}
                                        </th>
                                        <th colSpan={2} style={{ border: "1px solid #000", padding: "10px", textAlign: "center" }}>
                                            Styles: {data.flatMap(item => item.whReqItems.flatMap(reqItem => reqItem.whReqItemAttrs.styles)).join(', ')}
                                        </th>
                                    </tr>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    {/* {data.flatMap(item =>
                                        item.whReqItems.map(reqItem => {
                                            ...
                                            const locations = reqItem.itemsInfo.map(item => item.location?item.location:"NL").join(', ');
                                            const barcodeCount = reqItem.itemsInfo.reduce((count, item) => count + item.barcode.split(',').length, 0);
                                            const ctnQty = reqItem.itemsInfo.reduce((total, item) => total + item.ctnQty, 0);
...
export default FabricCheckListReport;
