import { FgWhHeaderIdReqDto, FgWhReqHeaderModel, FgWhSrIdPlIdsRequest, PkmsFgWhReqTypeEnum } from "@xpparel/shared-models";
...
interface Props {
    whReqHeaderId: number
}

export const DeliveryChalanPrint = ({ whReqHeaderId }: Props) => {
    ...
    useEffect(() => {
        getPackListIdsForHeaderReqId()
    }, []);

    const getPackListIdsForHeaderReqId = () => {
        const req = new FgWhHeaderIdReqDto(user?.userName, user?.orgData?.unitCode, user?.orgData?.companyCode, user?.userId, whReqHeaderId)
        fgWHService.getPackListIdsForHeaderReqId(req).then((res) => {
            if (res.status) {
                renderPrintDetails(res.data.packListIds)
            }
        }).catch(err => console.log(err.message))
    }

    const renderPrintDetails = async (plIds: number[]) => {
    ...
    const addresses: string[] = data?.flatMap(req =>
            ...
            <div id="deliver-chalan" style={{ fontFamily: "Arial, sans-serif", margin: "20px", padding: "10px" }}>
                {data && data.length > 0 && (
                    <div className="delivery-challan">
                        <h3 style={{ textAlign: "center", margin: "0" }}>SQ Birichina Ltd</h3>
...
export default DeliveryChalanPrint;
