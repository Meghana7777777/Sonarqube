...
export const LocationOutScan = ({ pendingCartonsDefault, onSessionEnd, whreqHeadId, selectedRecord, setSessionStarted, sessionStarted }: Props) => {
  ...
  const FgWarehouseInBarcodeLocationOut = async (value: string): Promise<CartonHeadInfoModel | null> => {
    try {
      ...
      const res = await service.FgWarehouseInBarcodeLocationOut(request);
  ...
  const handleBarcodeScanChange = async (value: string) => {
    try {
      const response = await FgWarehouseInBarcodeLocationOut(value);
      if (response) {
...
export default LocationOutScan;
