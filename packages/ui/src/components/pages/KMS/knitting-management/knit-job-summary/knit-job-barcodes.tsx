...
import Barcode4X2 from '../../../SPS/components/barcode'
import { BarcodeDetails, KC_KnitJobBarcodeModel } from '@xpparel/shared-models';
...
interface IKnitJobBarcodesProps {
  isModalOpen:boolean
  barcodesData: KC_KnitJobBarcodeModel[];
  printBarCodes: () => void;
  onClose:() => void;

}
export default function KnitJobBarcodes(props: IKnitJobBarcodesProps) {
  const [isModalOpen, setIsModalOpen] = useState(true);
  const handleOk = () => {
    setIsModalOpen(false);
  };

  console.log(props.barcodesData,'barcodes data')
  const handleCancel = () => {
    setIsModalOpen(false);
  };
            ...
            <div key={index} className="label">
              <table>
                <tbody>
                  ...
                  <tr> <td>Style code</td><td>: {barcode.features.styleCode || "NA"}</td>  </tr>
                  <tr> <td>Job No </td><td>: {barcode.jobNumber || "NA"} </td></tr>
                  ...
                  <tr> <td>MO No/ MO Lines</td> <td>: {barcode.features.moNumber || "NA"} / {(barcode.features.moLineNumber).join(', ') || "NA"}</td> </tr>
                  <tr> <td>Cusotmer Name</td> <td>: {barcode.features.customerName || "NA"}</td> </tr>
                  <tr> <td>Cusotmer No</td> <td>: {barcode.features.coNumber || "NA"}</td> </tr>
                  <tr> <td>Ex-Factory Date  </td> <td>  : {barcode.features.exFactoryDate} </td>  </tr>
