...
import { Select, Checkbox, Button, Modal, Tag, Collapse, Input, Form } from 'antd';
import OperationRoutingTableForCut from './operation-rounting-table-for-cut';
...
import { ComponentServices, ProductTypeServices, StyleProductOpService, StyleSharedService } from '@xpparel/shared-services';
import { AlertMessages } from '../../../common';
import { useAppSelector } from 'packages/ui/src/common';
import { IProcessDetails, IProcessTypeData, ISavedProcessData, ITblBasicData } from './op-routing-interface';
...
const OperationRoutingForCut = (props: IProps) => {
    const [selectedStyle, setSelectedStyle] = useState<string | null>(null);
    const [selectedProductType, setSelectedProductType] = useState<string | null>(null);
    const [selectedProcesses, setSelectedProcesses] = useState<string[]>([ProcessTypeEnum.CUT]);
    ...
    useEffect(() => {
        ...
        // getAllStyles();
        getAllProducts();
        setSelectedProcesses([ProcessTypeEnum.CUT]);
        setSavedData({});
        setVersionNo(null);
        setVersionDes('');
                ...
                <div style={{ background: '#e1e1e1', borderRadius: '10px', padding: '10px', width: "846px" }}>
                    ...
                    >
                        {processTypes.map((process) => (
                            <Checkbox key={process.value} value={process.value} disabled={process.value == ProcessTypeEnum.CUT}>
                                {process.label}
                            </Checkbox>
                        ))}
                    </Checkbox.Group>
...
export default OperationRoutingForCut;
