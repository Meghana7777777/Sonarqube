...
const PrintDispatch = (props: IProps) => {
    ...
    info.shippingItemsAbstractInfo.forEach(item => {
      item.dSetItemsAbstract.forEach(dSetAbstract => {
        dSetAbstract.dSetItems.sort((a, b) => 
          sizesOrderArray.indexOf(a.ctnBarcode) - sizesOrderArray.indexOf(b.ctnBarcode)
        );

        dSetAbstract.dSetItems.forEach((bundle, i) => {
          trs.push(
            <tr key={`${productIndex}-${i}-${dSetAbstract.moNo}${dSetAbstract.dSetId}`}>
              <td>{bundle.quantity}</td>
              <td>{dSetAbstract.destinations || 'No Product Name'}</td>
              <td>{dSetAbstract.dSetItemId}</td>
              <td>{bundle.quantity}</td> 
              <td>{bundle.quantity}</td>
              <td>{bundle.quantity}</td> 
              <td></td> 
