...
import { InsInspectionActivityStatusEnum, YarnInspectionBasicInfoModel, YarnTypeEnum } from "@xpparel/shared-models";
import { Form, Modal } from "antd";
import { useAppSelector } from "packages/ui/src/common";
import { useState } from "react";
import FourPointInspectionForm from "../../yarn-inspection-forms/yarn-inspection/yarn-inspection-form";
...
export const InProgressInsControl = (props: InProgressInsControlPorps) => {
    ...
    function getInspectionForm(irId: number, inspectionType: YarnTypeEnum) {
        // if (inspectionType == YarnTypeEnum.YARNINS) {
           
        // } else if (inspectionType == YarnTypeEnum.YARNINS) {
        //     return <ShadeInspectionForm inspReqId={irId} reloadParent={reloadDashboard} />
        // }
        //  else if (inspectionType == YarnTypeEnum.LAB_INSPECTION) {
        //     return <LabInspectionForm inspReqId={irId} reloadParent={reloadDashboard} />
        // } else if (inspectionType == YarnTypeEnum.SHRINKAGE) {
        //     return <Shrinkage inspReqId={irId} reloadParent={reloadDashboard} />
        // } else if (inspectionType == YarnTypeEnum.RELAXATION) {
    ...
    function renderInspectionUpdateModal(req: YarnInspectionBasicInfoModel) {
        // setShowModal(true)
    ...
    return (
        <>
            ...
            <Modal title='Yarn Inspection Update' width={'100vw'} style={{ top: 0, padding: 0 }} open={showModal} footer={null} onCancel={() => {
                setShowModal(false);
                // props.reloadDashboard();
            }}>
                {getInspectionForm(props.irInfo.irId, props.typeOfInspection)}
