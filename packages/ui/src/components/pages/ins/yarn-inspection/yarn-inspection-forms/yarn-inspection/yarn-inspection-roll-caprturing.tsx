import { CommonRequestAttrs, InsInspectionFinalInSpectionStatusEnum, PackFinalInspectionStatusEnum, YarnInsBasicInspectionRequest, YarnInsBasicInspectionRollDetails } from '@xpparel/shared-models';
...
export interface FourPointInspectionRollCapturingProps {
    rollsInfo: YarnInsBasicInspectionRequest;
    reloadParent?: () => void;
...
const YarnInspectionRollCapturingForm = (props: FourPointInspectionRollCapturingProps) => {
    const cartons: YarnInsBasicInspectionRollDetails[] = props?.rollsInfo?.inspectionRollDetails;
    ...
    const [rollLevelInputDetailsMap] = useState(new Map<number, YarnInsBasicInspectionRollDetails>);
   ...
   function renderInspectionResultSelection(rollInfo: YarnInsBasicInspectionRollDetails, index: number) {
    return (
        <Col span={8}>
            <div style={{ backgroundColor: '#fff', padding: '16px', borderRadius: '8px', boxShadow: '0 1px 4px rgba(0,0,0,0.1)', marginBottom: '20px' }}>
                <Form.Item label={<b>Yarn Inspection Defects</b>}>
...
function renderRollInspectionForm(rollInfo: YarnInsBasicInspectionRollDetails, index) {
        ...
        <div style={{
            ...
            <Row gutter={24}>
                <Col span={8}>
                    ...
                    <Form.Item label={<b>Yarn Count</b>} name={[index, 'yarnCount']}>
                        <InputNumber size="small" min={0} style={{ width: '100%' }} />
                    </Form.Item>

                    <Form.Item label={<b>Cone Final Result</b>} name={[index, 'rollFinalInsResult']}>
                        <Select
    ...
    function renderRollHeader(rollInfo: YarnInsBasicInspectionRollDetails) {
        console.log("header info", rollInfo);
        return <>
            <Space size={'large'}>
                <span><b>Cone Number</b>: {rollInfo.rollId}</span>
                <span><b>Cone Barcode</b>: {rollInfo.barcode}</span>
            </Space>
        </>
    }

    function renderCartons(rollsMap: YarnInsBasicInspectionRollDetails[]): CollapseProps['items'] {
...
export default YarnInspectionRollCapturingForm;
