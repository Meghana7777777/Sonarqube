...
import { CommonRequestAttrs, InsConeBarcodeInspCategoryReq, InsFabricInspectionRequestCategoryEnum, InsFabricInspectionRequestCategoryEnumDisplayValue, InsInspectionFinalInSpectionStatusEnum, InsInspectionResultEnum, InsIrIdRequest, InsRollBarcodeInspCategoryReq, PackFinalInspectionStatusEnum, YarnInsBasicInspectionRequest, YarnTypeEnum, YarnTypeEnumDisplayValue } from '@xpparel/shared-models';
import { FabricInspectionCaptureService, FabricInspectionInfoService, YarnInspectionCaptureService, YarnInspectionInfoService } from '@xpparel/shared-services';
...
import YarnInspectionRollCapturingForm from './yarn-inspection-roll-caprturing';
...
export const FourPointInspectionForm = (props: FourPointInspectionProps) => {
  ...
  const [fourPointInspectionDetails, setFourPointInspectionDetails] = useState<YarnInsBasicInspectionRequest>(null);
  ...
  const { Option } = Select;
  const yarnInspectionCaptureService = new YarnInspectionCaptureService();
  ...
  const getFourPointInspectionDetails = (unitCode: string, companyCode: string, userName: string, userId: number, inspReqId: number, rollBarcode: string) => {
    ...
    if (inspReqId) {
      ...
      yarnInspectionInfoService.getYarnInspectionDetailsForRequestId(reqObj, true).then((res) => {
        if (res.status) {
      ...
      const reqObj = new InsConeBarcodeInspCategoryReq(userName, unitCode, companyCode, userId, rollBarcode, YarnTypeEnum.YARNINS);
      yarnInspectionInfoService.getInspectionDetailForConeIdAndInspCategory(reqObj, true).then((res) => {
        if (res.status) {
          setBarcodeVal('');
          setManualBarcodeVal('');
          modifyAndSetInspectionData(res.data.basicInsInfo);
  ...
  const modifyAndSetInspectionData = (inspDetails: YarnInsBasicInspectionRequest) => {
  ...
  const onFinish = (val: YarnInsBasicInspectionRequest) => {
    try {
      ...
      const basicInsReq = new YarnInsBasicInspectionRequest();
  ...
  const saveInspectionDetails = (values) => {
    ...
    yarnInspectionCaptureService.captureInspectionResultsForYarn(formData).then((res) => {
      if (res.status) {
  ...
  function validateRolls(rolls: any[]): boolean {
    ...
    rolls.forEach((roll, index) => {
      if (
        ...
        isValid = false;
        AlertMessages.getErrorMessage(`Cone ${index + 1}: Inspection Not done, please verify`);
      }
    });
    return isValid;
  }
  ...
  return (
    ...
    <div >
      ...
      {fourPointInspectionDetails &&
        <ScxCard title={<span>Yarn Inspection</span>}
        >
                  ...
                  <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }} style={{ overflow: 'scroll' }}>
                    <Col lg={24} md={24} sm={24} xs={24}>
                      <Badge.Ribbon text={<span style={{ textAlign: "left" }}>Inspection request id: {fourPointInspectionDetails?.inspectionHeader?.inspectionReqId}</span>} color="red"  >
                        <ScxCard size='small' title={<div style={{ display: "flex", flexDirection: "row", justifyContent: "space-evenly", width: "100%" }}><span style={{ width: "10%" }}>Header Info</span><span style={{ width: "10%" }}>Inspection type : <b>{YarnTypeEnumDisplayValue[fourPointInspectionDetails?.inspectionHeader?.inspRequestCategory]}</b></span><Progress style={{ width: "10%" }} percent={inspCompPercentage} status="active" /></div>}>
                    ...
                    <>
                      <Row>
                        <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                          <YarnInspectionRollCapturingForm key={stateCounter} reloadParent={props.reloadParent} rollsInfo={fourPointInspectionDetails} />
                        </Col>
                      </Row>
                    </>
                  )}
