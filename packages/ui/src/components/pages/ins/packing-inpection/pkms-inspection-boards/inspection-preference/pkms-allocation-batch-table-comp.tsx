import { BatchInfoModel, InsInspReqStatusModel, PackingListInfoModel, RollInfoModel, RollSelectionTypeEnum } from '@xpparel/shared-models';
...
export const PKMSAllocationBatchTable = (props: IAllocationBatchTableProps) => {
    const { packListData, selectableInsRollsCount, selectedInsRolls, batchRolls, setSelectedInsRolls, disableSelectedRolls } = props;
    ...
    const getRowSelection = (batchNumber: string) => {
        ...
        if (packListData.rollSelectionType === RollSelectionTypeEnum.SYSTEMATIC) {
            obj['getCheckboxProps'] = (record: AllocationBatchTableModel) => ({
                disabled: true,
            });
        } else {
    ...
    const getItems = (batches: BatchInfoModel[]) => {
        const items: CollapseProps['items'] = batches.map(batch => {
            return {
                ...
                extra: <>
                    <Tag color="magenta">
                        Total Cartons : <b>{SequenceUtils.formatNumberToSpecificLength(`${batchRolls.get(batch.batchNumber.toString())?.length ? batchRolls.get(batch.batchNumber.toString())?.length : 0}`, 3)}</b>
                    </Tag>
                    <Tag color="#e27525">
                        Ins Cartons : <b>{SequenceUtils.formatNumberToSpecificLength(`${selectedInsRolls.get(batch.batchNumber)?.length ? selectedInsRolls.get(batch.batchNumber)?.length : 0}`, 3)}</b>
                    </Tag>
                    <Tag color="#7cb675">
                        Left Over Cartons : <b>{getPendingRolls(batchRolls.get(batch.batchNumber.toString())?.length, selectedInsRolls.get(batch.batchNumber)?.length)}</b>
                    </Tag>
                </>
            }
        });
