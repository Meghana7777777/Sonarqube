...
import { TrimTypeEnum, InsInspectionActivityStatusEnum, TrimInspectionBasicInfoModel, InsInspectionRequestsFilterRequest, InsInspectionTypeRequest, TrimInsInspectionRequestsFilterRequest, TrimInspectionTypeRequest, YarnInsInspectionRequestsFilterRequest } from "@xpparel/shared-models";
...
import { InspectionInfoCard } from "./trim-inspection-info-card";
...
export const InspectionColumnarCard = (props: InspectionColumnarCardProps) => {
	...
	function getIRBasicInfos(companyCode: string, unitCode: string, insType: TrimTypeEnum, insProgress: InsInspectionActivityStatusEnum, loadall: boolean) {

		let req: TrimInspectionTypeRequest;
		if (loadall) {
			req = new TrimInspectionTypeRequest(null, unitCode, companyCode, 0, insType, insProgress, 0, 0);

		}
		else {
			req = new TrimInspectionTypeRequest(null, unitCode, companyCode, 0, insType, insProgress, fromRecord.current, TOTAL_REC_COUNT);

		}

		trimInspectionInfoService.getInspectionMaterialPendingData(req).then(res => {
			if (res.status) {
				setColor(false)
				//filtering data 
				setIrsInfo(prev => {
	...
	function fetchInspectionDataByBatchCode(companyCode: string, unitCode: string, insType: TrimTypeEnum, insProgress: InsInspectionActivityStatusEnum, searchData: string) {

		let req: TrimInsInspectionRequestsFilterRequest;

		req = new TrimInsInspectionRequestsFilterRequest(null, unitCode, companyCode, 0, insType, insProgress, searchData);


		trimInspectionInfoService.getTrimInspectionRequestBasicInfoByLotCode(req).then(res => {
			if (res.status) {
				setIrsInfo([]);
	...
	//for filtering data
	function filterIrsInfo(newData: TrimInspectionBasicInfoModel[], existingData: TrimInspectionBasicInfoModel[]) {
		const ids = existingData.map(rec => rec.irId);
		const nonExistData: TrimInspectionBasicInfoModel[] = [];
	...
	function renderIrCards(irs: TrimInspectionBasicInfoModel[]) {
		return (
			<>
				{
					irs.map(req => {
