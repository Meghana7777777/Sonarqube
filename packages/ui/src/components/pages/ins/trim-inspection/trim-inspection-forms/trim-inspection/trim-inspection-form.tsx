...
import { CommonRequestAttrs, InsBoxBarcodeInspCategoryReq, InsFabricInspectionRequestCategoryEnum, InsInspectionFinalInSpectionStatusEnum, InsInspectionResultEnum, InsIrIdRequest, InsRollBarcodeInspCategoryReq, PackFinalInspectionStatusEnum, TrimInsBasicInspectionRequest, TrimTypeEnum } from '@xpparel/shared-models';
import { FabricInspectionCaptureService, FabricInspectionInfoService, TrimInspectionCaptureService, TrimInspectionInfoService } from '@xpparel/shared-services';
...
import TrimInspectionRollCapturingForm from './trim-inspection-roll-caprturing';
...
export const FourPointInspectionForm = (props: FourPointInspectionProps) => {
  ...
  const [fourPointInspectionDetails, setFourPointInspectionDetails] = useState<TrimInsBasicInspectionRequest>(null);
  ...
  const { Option } = Select;
  const trimInspectionCaptureService = new TrimInspectionCaptureService();
  ...
  const getFourPointInspectionDetails = (unitCode: string, companyCode: string, userName: string, userId: number, inspReqId: number, rollBarcode: string) => {
    ...
    if (inspReqId) {
      ...
      trimInspectionInfoService.getTrimInspectionDetailsForRequestId(reqObj, true).then((res) => {
        if (res.status) {
      ...
      const reqObj = new InsBoxBarcodeInspCategoryReq(userName, unitCode, companyCode, userId, rollBarcode, TrimTypeEnum.TRIMINS);
      trimInspectionInfoService.getInspectionDetailForBoxIdAndInspCategory(reqObj, true).then((res) => {
        if (res.status) {
          setBarcodeVal('');
          setManualBarcodeVal('');
          modifyAndSetInspectionData(res.data.basicInsInfo);
  ...
  const modifyAndSetInspectionData = (inspDetails: TrimInsBasicInspectionRequest) => {
  ...
  // function validateRollLevelCapturings(rollsInfo: TrimInsBasicInspectionRollDetails[]) {
  ...
  const onFinish = (val: TrimInsBasicInspectionRequest) => {
    try {
      ...
      const basicInsReq = new TrimInsBasicInspectionRequest();
  ...
  const saveInspectionDetails = (values) => {
    ...
    trimInspectionCaptureService.captureInspectionResultsForTrim(formData).then((res) => {
      if (res.status) {
  ...
  function validateRolls(rolls: any[]): boolean {
    ...
    rolls.forEach((roll, index) => {
      ...
      if (!roll.trimName || roll.trimName.trim() === '') missingFields.push('trimName');
      if (!roll.trimType || roll.trimType.trim() === '') missingFields.push('trimType');
      if (!roll.functionalChecks || roll.functionalChecks.trim() === '') missingFields.push('functionalChecks');
      if (!roll.visualChecks || roll.visualChecks.trim() === '') missingFields.push('visualChecks');
      if (!roll.colorChecks || roll.colorChecks.trim() === '') missingFields.push('colorChecks');
      if (!roll.strengthChecks || roll.strengthChecks.trim() === '') missingFields.push('strengthChecks');
      if ((roll.qualityPassed === 0 && roll.qualityFailed === 0)) missingFields.push('qualityPassed/Failed');
      if (!roll.trimFinalInsResult || roll.trimFinalInsResult.trim() === '') missingFields.push('trimFinalInsResult');

      if (missingFields.length > 0) {
        isValid = false;
        AlertMessages.getErrorMessage(
          `Box ${index + 1} (ID: ${roll.rollId}) is incomplete. Please fill the following fields: ${missingFields.join(', ')}`
        );
      }
    });
  ...
  return (
    ...
    <div >
      ...
      {fourPointInspectionDetails &&
        <ScxCard title={<span>Trim Inspection</span>}
        >
          <Form form={form} onFinish={saveInspectionDetails} initialValues={fourPointInspectionDetails}>
            ...
            <Col span={20}>
              ...
              <br />
              <ScxCard title={<span> Box level inspection</span>}>
                <Form.List name="inspectionRollDetails">
                  {(fields, { add, remove }) => (
                    <>
                      <Row>
                        <Col xs={24} sm={24} md={24} lg={24} xl={24} xxl={24}>
                          <TrimInspectionRollCapturingForm key={stateCounter} reloadParent={props.reloadParent} rollsInfo={fourPointInspectionDetails} />
                        </Col>
                      </Row>
                    </>
                  )}
