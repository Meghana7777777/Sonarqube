...
import { InsInspectionActivityStatusEnum, ThreadInsInspectionRequestsFilterRequest, ThreadInspectionBasicInfoModel, ThreadInspectionTypeRequest, ThreadTypeEnum } from "@xpparel/shared-models";
...
export const InspectionColumnarCard = (props: InspectionColumnarCardProps) => {
	...
	function getIRBasicInfos(companyCode: string, unitCode: string, insType: ThreadTypeEnum, insProgress: InsInspectionActivityStatusEnum, loadall: boolean) {

		let req: ThreadInspectionTypeRequest;
		if (loadall) {
			req = new ThreadInspectionTypeRequest(null, unitCode, companyCode, 0, insType, insProgress, 0, 0);

		}
		else {
			req = new ThreadInspectionTypeRequest(null, unitCode, companyCode, 0, insType, insProgress, fromRecord.current, TOTAL_REC_COUNT);

		}

		threadInspectionInfoService.getThreadInspectionMaterialPendingData(req).then(res => {
			if (res.status) {
				//filtering data 
				setColor(false)
				setIrsInfo(prev => {
	...
	function fetchInspectionDataByBatchCode(companyCode: string, unitCode: string, insType: ThreadTypeEnum, insProgress: InsInspectionActivityStatusEnum, searchData: string) {


		let req: ThreadInsInspectionRequestsFilterRequest;

		req = new ThreadInsInspectionRequestsFilterRequest(null, unitCode, companyCode, user.userId, insType, insProgress, searchData);

		threadInspectionInfoService.getThreadInspectionRequestBasicInfoByLotCode(req).then(res => {
			if (res.status) {
				setIrsInfo([]);
	...
	//for filtering data
	function filterIrsInfo(newData: ThreadInspectionBasicInfoModel[], existingData: ThreadInspectionBasicInfoModel[]) {
		const ids = existingData.map(rec => rec.irId);
		const nonExistData: ThreadInspectionBasicInfoModel[] = [];
	...
	function renderIrCards(irs: ThreadInspectionBasicInfoModel[]) {
		return (
			<>
				{
					irs.map(req => {
