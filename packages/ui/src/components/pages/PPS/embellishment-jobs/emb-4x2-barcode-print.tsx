...
import { PoEmbHeaderModel, RollInfoModel } from '@xpparel/shared-models';
import { Button, Modal, QRCode, Space } from 'antd';
import Barcode from 'react-barcode';
import { getCssFromComponent } from '../../WMS';

interface BarcodeProps {
    embBarcodesData: PoEmbHeaderModel[];
    style: string;
    venderName: string;
    printBarCodes: () => void
}

const EmbBarcode4X2 = (props: BarcodeProps) => {
        ...
        <div>
            ...
            <Modal title={<Space>
                ...
                <div id='printArea' style={{ width: '384px' }}>
                    {props.embBarcodesData.map(embObj => {
                        return embObj.embLines.map(embLine => {
                            return embLine.bundlesInfo.map(b => {
                                return <div className="label">
                                    <table>
                                        <tbody>
                                            ...
                                            <tr><td>Plant Style Ref </td><td>: {embObj.plantRefStyle}</td></tr>
                                            <tr><td>Cut No / Bundle No</td><td>: {embLine.panelFormEmbProps.cutSubNumber} / {b.underLayBundleNo}</td></tr>
                                            <tr><td>Color</td><td>: {embLine.panelFormEmbProps.color.slice(0, 30)}</td></tr>
                                            <tr><td>Size/Qty/Shade</td><td>: {b.size}/{b.quantity}/{b.shade}</td></tr>
                                            <tr><td>Component/Vendor </td><td>: {embLine.panelFormEmbProps.components.join()}/{props.venderName.slice(0, 20)}</td></tr>
                                            <tr><td>SO No/ SO Lines</td><td>: {embObj.moNo} / {embObj.moLines.join().slice(0, 40)}</td></tr>                                            
                                            ...
                                            /></td>
                                                <td>Op Code</td><td>: {embObj.operations.join()}</td>
                                            </tr>
                                            <tr><td>Garment PO / Item</td><td>: {embObj.garmentPo} /  {embObj.garmentPoItem}</td></tr>
