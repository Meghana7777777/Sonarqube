...
import { SewSerialBundleGroupReq, SewingJobSummaryForSewingOrder, SewingOrderReq } from "@xpparel/shared-models";
...
interface IGridProps {
    sewingOrderNo?: number;
    bundleGroupNo?: number;
    updateKey: number;
    sewJobSummaryData?: SewingJobSummaryForSewingOrder;
}
...
const SewingPlannedSummaryGrid = (props: IGridProps) => {
    ...
    useEffect(() => {
        if (props.sewingOrderNo) {
            getSewingCutDocketInfoForJobFeatures();
        }
        if (props.sewJobSummaryData) {
            setSewCutOrderInfo(props.sewJobSummaryData);
            processData(props.sewJobSummaryData);
        }
    ...
    const getSewingCutDocketInfoForJobFeatures = () => {
        ...
        const req = new SewSerialBundleGroupReq(user?.userName, user?.orgData?.unitCode, user?.orgData?.companyCode, user?.userId, props.sewingOrderNo, props.bundleGroupNo);
        sewingJobGenMOService.getSewingJobSummaryForSewingOrderAndBundleGroup(req).then((res => {
            if (res.status) {
...
export default SewingPlannedSummaryGrid;