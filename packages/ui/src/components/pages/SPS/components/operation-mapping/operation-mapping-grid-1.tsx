...
// import {  OpFormEnum, OpVRawMaterial, OperationCategoryFormRequest, PoProdutNameRequest, OpCategoryEnum, RawMaterialIdRequest, SewVerIdRequest, SewVersionModel, SewVersionRequest, UomEnum, VendorCategoryEnum, VendorCategoryRequest, WarehouseIdRequest, opCategoryEnumDisplayValues } from "@xpparel/shared-models";
import { InsUomEnum, OpFormEnum, OpVRawMaterial, OperationCategoryFormRequest, PoProdutNameRequest, ProcessTypeEnum, processTypeEnumDisplayValues, RawMaterialIdRequest, SewVerIdRequest, SewVersionModel, SewVersionRequest, VendorCategoryRequest, WarehouseIdRequest } from "@xpparel/shared-models";
import { OperationService, SewingMappingService, VendorService, WarehouseSharedService } from "@xpparel/shared-services";
...
import JobOrderFlow from "./JobOrderFlowChart";
const { Option } = Select
...
const OperationMappingGrid = (props: IProps) => {
    ...
    const sewMappingService = new SewingMappingService();
    ...
    useEffect(() => {
        if (props?.poSerial) {
            ...
            getSewVersionForPoProductName(props?.poSerial, props?.productName);
        }
    }, [])

    useEffect(() => {
        const opversiondata = new SewVersionModel(null, "verisonname", "description", opVersionData, opGroupsData, null, null)

        console.log(opversiondata, "Material required")
                    ...
                    <Row gutter={[16, 16]} align="middle" justify="start" style={{ marginBottom: "15px", marginLeft: '20px' }}>
                        <Col span={4}>
                            ...
                            <Select
                                ...
                                style={{ width: '100%' }}
                            >
                                {Object.values(ItemCategoryEnum)?.map((product) => (
                        ...
                        <Col span={4}>
                            ...
                            <Select
                                ...
                                style={{ width: '100%' }}
                            >
                                {Object.values(RawMaterialEnum)?.map((materialType) => (
    ...
    const getSewVersionForPoProductName = (poSerial: number, productName: string) => {
        const req = new PoProdutNameRequest(user?.userName, user?.orgData?.unitCode, user?.orgData?.companyCode, user?.userId, poSerial, productName, null)
    ...
    const handleItemCodeChange = (record,val, index) => {
        ...
        opGroupsData[index].components = itemCodeComponents[record]
        setOpGroupsData([...opGroupsData])
                        ...
                        <span>
                            {record.groupCategory === ProcessTypeEnum.KNIT?(<>
                                ...
                                <Form.Item
                                    ...
                                    initialValue={itemCodeComponents[record.itemCode]}  
                                    rules={[
                                        {
                                            required: true,
                                ...
                                >
                                    <Select
                                        ...
                                        style={{ width: '450px' }}
                                        mode="multiple"
                                        value={itemCodeComponents[record.itemCode]} // Make sure components are selected by default
                                    ...
                                    >
                                        {itemCodeComponents[record.itemCode]?.map((component) => (
    ...
    const handlePreview=()=> {
        opGroupsForm.validateFields().then(() => {
            pageForm.validateFields().then(() => {
                ...
                const sewVersionReq = new SewVersionRequest(user?.userName, user?.orgData?.unitCode, user?.orgData?.companyCode, user?.userId, props.productName,0, opversiondata, rawMaterials,null);
    ...
    const saveSequence = (val) => {
        ...
        opGroupsForm.validateFields().then(() => {
            pageForm.validateFields().then(() => {
                ...
                const sewVersionReq = new SewVersionRequest(user?.userName, user?.orgData?.unitCode, user?.orgData?.companyCode, user?.userId, props.productName, props?.poSerial, opversiondata, rawMaterials);
                ...
                <Row gutter={24}>
                    ...
                    {!isOpSeqExists ?
                        ...
                        <div style={{display:'flex',gap:'10px'}}>
                            ...
                            <Modal width={1200} open={showPreviewModal} onCancel={()=>setShowPreviewModal(false)}
                            >
                            <JobOrderFlow jobOrderFlowData={flowChartData}></JobOrderFlow>
                            </Modal>
...
export default OperationMappingGrid;