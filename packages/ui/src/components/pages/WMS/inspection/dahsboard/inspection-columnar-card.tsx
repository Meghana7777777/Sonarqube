...
import { InsFabricInspectionRequestCategoryEnum, InsInspectionActivityStatusEnum, InsInspectionBasicInfoModel, InsInspectionRequestsFilterRequest, InsInspectionTypeRequest } from "@xpparel/shared-models";
...
export const InspectionColumnarCard = (props: InspectionColumnarCardProps) => {
	...
	function getIRBasicInfos(companyCode: string, unitCode: string, insType: InsFabricInspectionRequestCategoryEnum, insProgress: InsInspectionActivityStatusEnum, loadall: boolean) {

		let req: InsInspectionTypeRequest;
		if (loadall) {
			req = new InsInspectionTypeRequest(null, unitCode, companyCode, 0, insType, insProgress, 0, TOTAL_REC_COUNT);

		}
		else {
			req = new InsInspectionTypeRequest(null, unitCode, companyCode, 0, insType, insProgress, fromRecord.current, TOTAL_REC_COUNT);

		}

		inspectionInfoService.getInspectionRequestBasicInfo(req).then(res => {
			if (res.status) {
				//filtering data 
				setColor(false)
				setIrsInfo(prev => {
	...
	function fetchInspectionDataByBatchCode(companyCode: string, unitCode: string, insType: InsFabricInspectionRequestCategoryEnum, insProgress: InsInspectionActivityStatusEnum, searchData: string) {

		let req: InsInspectionRequestsFilterRequest;

		req = new InsInspectionRequestsFilterRequest(null, unitCode, companyCode, user.userId, insType, insProgress, searchData);

		inspectionInfoService.getInspectionRequestBasicInfoByBatchCode(req).then(res => {
			if (res.status) {
				setIrsInfo([]);
	...
	//for filtering data
	function filterIrsInfo(newData: InsInspectionBasicInfoModel[], existingData: InsInspectionBasicInfoModel[]) {
		const ids = existingData.map(rec => rec.irId);
		const nonExistData: InsInspectionBasicInfoModel[] = [];
	...
	function renderIrCards(irs: InsInspectionBasicInfoModel[]) {
		return (
			<>
				{
					irs.map(req => {
