...
import { InsFabricInspectionRequestCategoryEnum, InsInspectionActivityStatusEnum, InsInspectionBasicInfoModel, InsIrActivityChangeRequest } from "@xpparel/shared-models";
import { FabricInspectionCreationService } from "@xpparel/shared-services";
...
export const PendingInsControl = (props: PendingInsControlProps) => {
    ...
    const [inspectionBasicInfo, setInspectionBasicInfo] = useState<InsInspectionBasicInfoModel>();


    // -------------------------------- APIS START-------------------------------
    function confirmMaterialReceivedForInspection(req: InsInspectionBasicInfoModel, date: string, remarks: string) {
        ...
        insService.updateInspectionActivityStatus(irConfReq).then(res => {
            if (res.status) {
    ...
    function renderInspectionMaterialConfirmationModal(req: InsInspectionBasicInfoModel) {
    ...
    function reRequestConfirmingModel(req: InsInspectionBasicInfoModel) {
    ...
    return (
        <>
            ...
            onCancel={()=>{
                ...
                closable={true}
            >
                <IrRollsSummary  key={props.irInfo?.irId} irInfo={props.irInfo} inspectionViewProgress={props.inspectionViewProgress} typeOfInspection={props.typeOfInspection} />
            </Modal>
        </>
    );
};
