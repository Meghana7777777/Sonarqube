...
import { InsFabricInspectionRequestCategoryEnum, InsInspectionActivityStatusEnum, InsInspectionBasicInfoModel } from "@xpparel/shared-models";
import { Form, Modal } from "antd";
import { useAppSelector } from "packages/ui/src/common";
import { useState } from "react";
import { Shrinkage } from "../../inspection-forms";
import FourPointInspectionForm from "../../inspection-forms/four-point-inspection/four-point-inspection-form";
import LabInspectionForm from "../../inspection-forms/lab-inspection-form";
import RelaxationInspectionForm from "../../inspection-forms/relaxation-inspection-form";
import ShadeInspectionForm from "../../inspection-forms/shade-inspection-form";
...
export const InProgressInsControl = (props: InProgressInsControlPorps) => {
    ...
    function getInspectionForm(irId: number, inspectionType: InsFabricInspectionRequestCategoryEnum) {
        if (inspectionType == InsFabricInspectionRequestCategoryEnum.INSPECTION) {
            return <FourPointInspectionForm inspReqId={irId} reloadParent={reloadDashboard} reload={reloadCounter} />
        } else if (inspectionType == InsFabricInspectionRequestCategoryEnum.SHADE_SEGREGATION) {
            return <ShadeInspectionForm inspReqId={irId} reloadParent={reloadDashboard} reload={reloadCounter} />
        } else if (inspectionType == InsFabricInspectionRequestCategoryEnum.LAB_INSPECTION) {
            return <LabInspectionForm inspReqId={irId} reloadParent={reloadDashboard} reload={reloadCounter}/>
        } else if (inspectionType == InsFabricInspectionRequestCategoryEnum.SHRINKAGE) {
            return <Shrinkage inspReqId={irId} reloadParent={reloadDashboard} reload={reloadCounter}/>
        } else if (inspectionType == InsFabricInspectionRequestCategoryEnum.RELAXATION) {
            return <RelaxationInspectionForm inspReqId={irId} reloadParent={reloadDashboard} reload={reloadCounter}/>
        } else {
    ...
    function renderInspectionUpdateModal(req: InsInspectionBasicInfoModel) {
        // setShowModal(true)
    ...
    function renderModalBasedOnSelection(currentModalDefaultConfigs: CurrentModalOptions, req: InsInspectionBasicInfoModel, insType: InsFabricInspectionRequestCategoryEnum) {
    ...
    function renderIrItemsInfoModal(req: InsInspectionBasicInfoModel, insType: InsFabricInspectionRequestCategoryEnum) {
    ...
    return (
        <>
            ...
            <Modal title='Inspection Update' width={'100vw'} style={{ top: 0, padding: 0 }} open={showModal} footer={null} onCancel={() => {
                setShowModal(false);
                // props.reloadDashboard();
            }}>
                {getInspectionForm(props.irInfo.irId, props.typeOfInspection)}
