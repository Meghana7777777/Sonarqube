import { InsCommonInspectionHeaderInfo, InsInspectionFinalInSpectionStatusEnum } from '@xpparel/shared-models';
...
export const InspectionHeaderForm = (props: InspectionHeaderFormProps) => {
    ...
    return <div>
        <Descriptions size='small' bordered column={{ xxl: 4, xl: 3, lg: 3, md: 2, sm: 2, xs: 1 }}>
            ...
            <Descriptions.Item label={<b>{'Request age'}</b>}><span>{calculateDateDifference(new Date(moment(inspectionHeader?.inspectedDate).format(defaultDateFormat)), new Date())}</span></Descriptions.Item>
