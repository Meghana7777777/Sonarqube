...
interface Props {
  ...
  requestDetails: IInventoryInfo[];
  iNeedActionItems: boolean;
}

const IPSInventoryAllocationsTablePage: React.FC<Props> = ({ jobNumber, processType, refreshKey, allocations, requestDetails, iNeedActionItems }) => {
  ...
  const handlePrintBundles = async (allocationId: number, issuedDate: string, issuedBy: string) => {
    try {
      ...
      if (response.data && response.data.length > 0) {
        ...
        message.warning('No Inv Allocations found to print.');
      }
    } catch (error) {
      AlertMessages.getErrorMessage('Failed to print bundles');
    }
  };
  ...
  const getMergedData = () => {
    return requestDetails?.map((request) => {
      return {
  ...
  const columns: ColumnsType<any> = [
    ...
    // },
    {
      ...
      render: (issued: boolean) => {
        ...
        const text = issued ? 'Received' : 'Requested';
        return <Tag color={color}>{text}</Tag>;
      }
    },
    {
  ...
  return (
    <Card size='small' title={<span style={{ display: 'flex', justifyContent: 'center' }}>Inventory Allocation</span>} bordered>
      <Table
        columns={columns}
        ...
        locale={{ emptyText: 'No Inv allocations found' }}
      />
...
export default IPSInventoryAllocationsTablePage;
