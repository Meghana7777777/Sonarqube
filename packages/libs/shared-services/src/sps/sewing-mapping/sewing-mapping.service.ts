import { GlobalResponseObject, JobGroupBundleGroupProcessTypeResponse, JobGroupVersionInfoResp, PoProdNameResponse, PoProdutNameRequest, PoSerialRequest, RawMaterialIdRequest, SewRawMaterialResponse, SewSerialOpsResponse, SewSerialRequest, SewVerIdRequest, SewVersionCloneRequest, SewVersionRequest, SewVersionResponse } from '@xpparel/shared-models';
import { AxiosRequestConfig } from 'axios';
import { SPSCommonAxiosService } from '../sps-common-axios.service';



export class SewingMappingService extends SPSCommonAxiosService {
    ...
    async getProdcutWiseSewOpVersionsForSewSerial(req: SewSerialRequest, config?: AxiosRequestConfig): Promise<SewSerialOpsResponse> {
        return await this.axiosPostCall(this.getURLwithMainEndPoint('getProdcutWiseSewOpVersionsForSewSerial'), req, config);
    }

    async getGroupAndProcessTypeInfoBySewSerial(req: SewSerialRequest, config?: AxiosRequestConfig): Promise<JobGroupBundleGroupProcessTypeResponse> {
        return await this.axiosPostCall(this.getURLwithMainEndPoint('getGroupAndProcessTypeInfoBySewSerial'), req, config);
    }
