...
export class FgContainerServices extends PKMSCommonAxiosService {
 private getURLwithMainEndPoint(childUrl: string) {
   ...
   async createContainers(reqModel: FgContainerCreateRequest, config?: AxiosRequestConfig):Promise<FgContainerResponse> {
        return await this.axiosPostCall(this.getURLwithMainEndPoint('createContainers'), reqModel, config);
    }

    async ActivateDeactivateContainers(reqModel: FgContainersActivateReq, config?: AxiosRequestConfig): Promise<FgContainerResponse> {
